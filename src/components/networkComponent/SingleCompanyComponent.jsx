...
function SingleCompanyComponent({selectedId, showSingleCompanyComp, setShowSingleComp}) {
    ...
    const company = itCompanies[selectedId -1 ]
  return (
    <div className='container-xl mx-auto companies'>
        <div className='d-flex justify-content-end my-3' onClick={handletogggle}>
            <button className='btn btn-success mt-4 ' >Back To Pages Tab</button>
        </div>
            
        <div class="row ">
            <div class="col-9">
                <div className="card border">
                    <img style={{height:"300px", objectFit:"cover"}} src={company.logoUrl} alt="" />
                    <div className='p-3 mt-3'>
                        <h1>{company.name}</h1>
                        <p>{company.description}</p>
                        <p>{company.industry} services - {company.location} - {company.followers} - {company.employees} Employees</p>
                        {/* <p className='d-flex flex-wrap'>{company.competitors.map((data) => <><h5 className=''>{data}, </h5></>)}</p> */}
                         <button className='btn btn-outline-primary rounded-pill'>Following</button>
                         <button className='btn btn-primary rounded-pill mx-2'>Visit Website</button>
                    </div>
                    <div className='border-top'>
                        <div className='d-flex flex-wrap '>
                            <p className=' mx-3 p-3 active'>Home</p>
                            <p className=' mx-3 p-3'>About</p>
                            <p className=' mx-3 p-3'>Post</p>
                            <p className=' mx-3 p-3'>Jobs</p>
                        </div>

                    </div> 
                </div>
                <div className='card mt-3 border p-3'>
                    <h4 className='fw-bold'>About this Company</h4>
                 <p>{company.shortDescription}</p>
                 <p>{company.description}</p>
                </div>
                <div className="row singlecompanyComponent">
                    <div className="col-3 p-3">
                      <div ref={stickyCardRef} style={{width:"230px"}} className={` ${isSticky ? 'sticky-top' : ''}`}>
                      <div className="card p-3">
                        <img src={company.logoUrl} style={{width:"300px", objectFit:"contain", width:"auto"}} className='rounded-4' alt="" />
                        <h3 className='fw-bold text-start'>{company.name}</h3>
                        <p>{company.website}</p>
             ...
             <div className="col-9 ">
                    {company.posts.map((data) => {
                                        ...
                                        <div>
                                          ...
                                          <div className="d-flex postedUser justify-content-between p-3">
                                            ...
                                            <div className='text-start mx-2' style={{fontSize:"12px"}}>
                                              ...
                                              <h6>{company.industry} sector | {company.headquarters}</h6>
                        ...
                        <path d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425a.247.247 0 0 1 .02-.022"/>
                                          ...
                                          {/* image */}
                                                  <div className=''>
                                                    <h5 className='text-start px-4'>{data.content}</h5>
                                                    <img className='w-100 my-4' src={data.imageUrl ? data.imageUrl : 'https://pbs.twimg.com/media/ErlasHvU0AEsM5w.jpg:large'} alt="" />
                                                  </div> 
                                        </div>
                                      </div>
                                  <div class="card-footer text-body-secondary">
                                  <div className="d-flex plate flex-wrap">
